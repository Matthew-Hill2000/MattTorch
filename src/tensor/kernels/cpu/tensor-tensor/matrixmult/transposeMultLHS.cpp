...
void packRhs(double* block, const double* rhs, int kBase, int nBase,
  ...
  for (; j + 7 < nMax; j += 8) {
    for (int k{0}; k < kMax; k++) {
      __m256d src0 = _mm256_loadu_pd(&rhs[(kBase + k) * rowStride + nBase + j]);
      __m256d src1 =
          _mm256_loadu_pd(&rhs[(kBase + k) * rowStride + nBase + j + 4]);
  ...
  if (j + 3 < nMax) {
    for (int k{0}; k < kMax; k++) {
      __m256d src = _mm256_loadu_pd(&rhs[(kBase + k) * rowStride + nBase + j]);
  ...
  for (; j < nMax; j++) {
    for (int k{0}; k < kMax; k++) {
      block[j * kMax + k] = rhs[(kBase + k) * rowStride + nBase + j];
    }
  }
}
...
void packLhs(double* block, const double* lhs, int mBase, int kBase,
  ...
  for (; i + 5 < mMax; i += 6) {
    for (int k{0}; k < kMax; k++) {
      const double* col = &lhs[(kBase + k) * rowStride + mBase + i];
      block[i * kMax + 6 * k + 0] = col[0];
      block[i * kMax + 6 * k + 1] = col[1];
      block[i * kMax + 6 * k + 2] = col[2];
      block[i * kMax + 6 * k + 3] = col[3];
      block[i * kMax + 6 * k + 4] = col[4];
      block[i * kMax + 6 * k + 5] = col[5];
    }
  }

  for (; i < mMax; i++) {
    for (int k{0}; k < kMax; k++) {
      block[i * kMax + k] = lhs[(kBase + k) * rowStride + mBase + i];
    }
  }
}
    ...
    for (int nBlock = 0; nBlock < N; nBlock += BIGBLOCKSIZE_N) {
      for (int mBlock = 0; mBlock < M; mBlock += BIGBLOCKSIZE_M) {
        for (int kBlock = 0; kBlock < K; kBlock += BIGBLOCKSIZE_K) {
          ...
          packRhs(rhsBigBlock, rhs, kBlock, nBlock, rhsCols, kMax, nMax);
