...
class GradMultiply : public GradFunction {
 private:
  ...
  // The GradFunction of the parents in the computational graph
  std::vector<std::shared_ptr<GradFunction>> nextFunctions;

 public:
  // Create a GradMultiply object with a vector containing the tensors that were
  ...
  // computation graph
  GradMultiply(std::vector<TensorView> savedTensors,
               std::vector<std::shared_ptr<GradFunction>> nextFunctions);
